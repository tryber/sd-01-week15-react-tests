...
import { getLocationPokemonAPI } from '../services/pokedexService';
...
class Locations extends Component {
  ...
  componentDidMount() {
    getLocationPokemonAPI().then((values) => this.setState({
      data: values.results,
      loading: false,
  ...
  async btnNext() {
    ...
    await getLocationPokemonAPI(counter).then((values) => {
      this.setState({
        data: values.results,
        loading: false,
