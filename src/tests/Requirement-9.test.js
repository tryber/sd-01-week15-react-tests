...
describe('Exigência → 9', () => {
  test(' O pokémon exibido na Pokedéx deve conter um link de navegação para exibir detalhes deste pokémon', () => {
    ...
    const proxPoker = getByText(/Próximo pokémon/i);
    pokemons.forEach((pokemon) => {
      const link = getByText(/More details/i).href;
      expect(link).toBe(`http://localhost/pokemons/${pokemon.id}`);
      fireEvent.click(proxPoker);
    });
  });
});
