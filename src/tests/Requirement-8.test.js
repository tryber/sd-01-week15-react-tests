...
describe('Exigência → 8', () => {
  test(' A Pokedéx deve exibir o nome, tipo, peso médio e imagem do pokémon exibido', () => {
    ...
    pokemons.forEach((pokemon) => {
      ...
      const pokemonImage = getByAltText(`${pokemon.name} ${alt}`);
      expect(averageWeight).toBe(pokemonWeight);
      expect(pokemonImage.src).toBe(pokemon.image);
      expect(pokemonImage.alt).toBe(`${pokemon.name} ${alt}`);
