...
describe('Exigência → 7', () => {
  test('O botão de Próximo pokémon deve ser desabilitado se a lista filtrada de pokémons tiver um só pokémon', () => {
    ...
    const btnProxPoker = getByText(/Próximo pokémon/i);
    ...
    pokemonTypes.forEach((type) => {
      ...
      const namePoker = getByText(/Average weight:/i).previousSibling.previousSibling.textContent;
      fireEvent.click(btnProxPoker);
      const proxPoker = getByText(/Average weight:/i).previousSibling.previousSibling.textContent;
      if (namePoker === proxPoker) expect(btnProxPoker.disabled).toBe(true);
      else expect(btnProxPoker.disabled).toBe(false);
    });
  });
});
